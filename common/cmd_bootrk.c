...
extern int firefly_hwversion_setenv(char *command_line, int size);
...
static void rk_commandline_setenv(const char *boot_name, rk_boot_img_hdr *hdr, bool charge)
{
	...
	else if(StorageSDCardBootMode()) { /* sdcard Boot */
		snprintf(command_line, sizeof(command_line),
				"%s %s", command_line, "androidboot.mode=sd");
	}
    ...
    firefly_hwversion_setenv(command_line, sizeof(command_line));
