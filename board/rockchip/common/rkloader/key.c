...
__maybe_unused static int rkkey_parse_recoverykey_dt(const void *blob);
#endif
...
#ifdef CONFIG_OF_LIBFDT
	if (gd->fdt_blob != NULL)
		rkkey_parse_recoverykey_dt(gd->fdt_blob);
#endif
#endif
}
...
__maybe_unused static void ChargeStateGpioInit(void)
{
	...
	charge_state_gpio.gpio = (GPIO_BANK0 | GPIO_A1);
	gpio_direction_output(charge_state_gpio.gpio, 0);
...
#ifdef CONFIG_OF_LIBFDT
static int rkkey_parse_recoverykey_dt(const void *blob)
{
	int recoverykey_node;

	recoverykey_node = fdt_node_offset_by_compatible(blob, -1, "rockchip,key");
	...
	if (recoverykey_node < 0) {
		printf("no rockusb key node\n");
		return -1;
	}

	key_rockusb.key.adc.keyValueLow = fdtdec_get_int(blob, recoverykey_node, "keyvalue-low", 0);
	key_rockusb.key.adc.keyValueHigh = fdtdec_get_int(blob, recoverykey_node, "keyvalue-high", 30);

	printf("rockusb key: key_rockusb.key.adc.keyValueLow = %d, key_rockusb.key.adc.keyValueHigh = %d\n", key_rockusb.key.adc.keyValueLow, key_rockusb.key.adc.keyValueHigh);
	return 0;
}
#endif /* CONFIG_OF_LIBFDT */
