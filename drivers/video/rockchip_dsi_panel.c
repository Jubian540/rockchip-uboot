...
struct rockchip_dsi_panel {
	...
	struct dsi_panel_cmds *read_id_cmds;
};
...
static int rockchip_dsi_panel_getId(struct display_state *state)
{
    ...
    while((ret < 0) && count > 0){
	  if(len > 1){
		max_size[0] = len;
 		ret = mipi_dsi_set_maximum_return_packet_size(state, max_size,1);
	  }
	  ret = mipi_dsi_dcs_read(state, 0x04, buf, len);
      count --;
    }
    if(ret == 0)
	    for(i = 0; i < len; i++){
	         printf("read buf id[%d] = %x,panel target id[%d] = %x   len = %d\n", i, *(buf + i), i, *(panel->id->buf + i), len);
	         //printf("read buf id[%d] = %x, len = %d\n", i, *(buf + i), i, len);
	         if(*(buf + i) != *(panel->id->buf + i))
			 	return -1;        
	}else{
	    printf("read panel(num = %d) timeout or no response\n",panel->num);
		panel_state->num = panel->num;
		return -1;
	}	
	panel_state->num = panel->num;
	return 0;
...
static int rockchip_dsi_panel_parse_dt(const void *blob, int node, struct display_state *state)
{
	//struct fdt_gpio_state *enable_gpio = &panel->enable_gpio;
	//struct fdt_gpio_state *reset_gpio = &panel->reset_gpio;
	struct rockchip_dsi_panel *panel = state->panel_state.private;
	...
	panel->id_reg =  fdtdec_get_int(blob, node, "id-reg", 0);
    panel->num = fdtdec_get_int(blob, node, "num", 0);
	data = fdt_getprop(blob, node, "id", &len);
	if(data){
		panel->id = malloc(sizeof(*panel->id));
		if (!panel->id)
			return -ENOMEM;
		panel->id->buf = malloc(sizeof(char) * len);		
		memcpy(panel->id->buf, data, len);
		panel->id->len = len;
	}
	...
	list_for_each(pos, pclist_head) {
		...
		if(pwrctr->debug > 0) {
			if(!strcmp(pclist->pc.name, "lcd-55power"))
				panel->enable_gpio = pclist->pc.gpio.gpio;
			if(!strcmp(pclist->pc.name, "lcd-rst"))
				panel->reset_gpio = pclist->pc.gpio.gpio;
		}
	}
...
static int rockchip_dsi_panel_init(struct display_state *state)
{
	...
	ret = rockchip_dsi_panel_parse_dt(blob, node, state);
	if (ret) {
