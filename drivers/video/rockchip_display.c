...
static int connector_panel_init(struct display_state *state,bool enable)
{
	struct connector_state *conn_state = &state->conn_state;
	struct panel_state *panel_state = &state->panel_state;
	const struct rockchip_connector *conn = state->conn_state.connector;
   ...
   if(enable){
		...
		panel_state->num = -1;
		for (panel_node = fdt_subnode_offset(blob, conn_node, "panel");
		     panel_node >= 0;
		    ...
		    if(rockchip_panel_getId(state) != -1){
...
static int display_get_timing(struct display_state *state)
{
	...
	panel = panel_state->node > 0 ? panel_state->node : get_panel_node(state, conn_node);
	if (panel < 0) {
		printf("failed to find panel node\n");
		return -ENODEV;
	}
...
int rockchip_display_init(void)
{
	...
	int route, child, phandle, connect, crtc_node, conn_node, panel_node;
	int count;
	...
	fdt_for_each_subnode(blob, child, route) {
		...
		for (panel_node = fdt_subnode_offset(blob, conn_node, "panel");
	       panel_node >= 0;
	       panel_node = fdt_next_subnode(blob, panel_node)){
	            s->conn_state.phy_txrequestclkhs =  fdtdec_get_int(blob,  panel_node,
					"phy-txrequestclkhs", 0) != 0 ? 1 : 0;

	            count++;
		}
		if(count > 1)
		   connector_panel_init(s, true);
		else
		   connector_panel_init(s, false);
