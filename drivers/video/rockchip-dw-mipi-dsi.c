...
#define TEAR_FXEN			BIT(0)
#define EN_ACK_RQST			BIT(1)
#define EN_TEAR_FX			BIT(0)

#define CMD_MODE_ALL_LP			(MAX_RD_PKT_SIZE | \
					 DCS_LW_TX | \
					 DCS_SR_0P_TX | \
					 DCS_SW_1P_TX | \
					 DCS_SW_0P_TX | \
...
struct dw_mipi_dsi {
	...
	unsigned int phy_txrequestclkhs;
	/* dual-channel */
...
static void dw_mipi_dsi_set_transfer_mode(struct dw_mipi_dsi *dsi, int flags)
{
	if (dsi->mode_flags & MIPI_DSI_MODE_LPM) {
		...
		dsi_update_bits(dsi, DSI_LPCLK_CTRL, PHY_TXREQUESTCLKHS, dsi->phy_txrequestclkhs);
	} else {
...
static int dw_mipi_dsi_connector_init(struct display_state *state)
{
	...
	dsi->state = state;
	dsi->phy_txrequestclkhs = conn_state->phy_txrequestclkhs;
	...
	panel = panel_state->node > 0 ? panel_state->node : fdt_subnode_offset(state->blob, mipi_node, "panel");
	if (panel < 0) {
...
static int fdt_fixup_display_panel_status(void *blob, const char *name,
                        ...
                        int num)
{
    ...
    if(!blob || !name)
		return -1;
	connector = fdt_path_offset(blob, name);
	for (panel_node = fdt_subnode_offset(blob, connector, "panel");
	     ...
	     if(fdtdec_get_int(blob, panel_node, "num", 0) != num){
	          ...
	          printf("fixup panel(num=%d) disabled\n",fdtdec_get_int(blob, panel_node, "num", 0));
	      	  /* fixup panel info */
		      fdt_fixup_node_status(blob, panel_node, status);
	     }
	 }
	return 0;
}
...
static int dw_mipi_dsi_connector_fixup_dts(struct display_state *state, void *blob)
{
	struct connector_state *conn_state = &state->conn_state; 
	struct panel_state *panel_state = &state->panel_state;
	int num = panel_state->num;
	char path[100];	
	int ret;
	
	fdt_get_path(state->blob, conn_state->node, path, sizeof(path));	
	ret = fdt_fixup_display_panel_status(blob, path, FDT_STATUS_DISABLED, num);
	if(ret == -1)
		printf("fixup panel status failed!\n");
